...
def _archive_card(rec: dict) -> str:
    ...
    badge = ('<span class="ar-badge sample">Sample</span>' if is_sample
             else '<span class="ar-badge live">Live</span>')
    return (
...
<h2>Archive</h2>
<div class="filterbar"><select id="f-market" aria-label="시장 필터"><option value="">All markets</option><option value="KR">Korea</option><option value="US">U.S.</option></select><select id="f-window" aria-label="윈도 필터"><option value="">All windows</option><option value="preopen">장 시작 전</option><option value="close">장 마감</option></select></div>
